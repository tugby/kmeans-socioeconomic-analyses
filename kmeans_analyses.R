...
#Veri Tipini Duzeltme ve Numeric Yapma
...
veri$intentional_homicides <- as.numeric(as.character(veri$intentional_homicides))
...
#NORMAL DAGILIM ANALIZI
par(mfrow=c(2,2))

hist(veri$gdp, main="GSYH (GDP) Histogram", col="skyblue", xlab="Deger", ylab="Frekans")
hist(veri$military, main="Askeri (Military) Histogram", col="plum", xlab="Deger", ylab="Frekans")
hist(veri$intentional_homicides, main="Cinayet (Homicides) Histogram", col="salmon", xlab="Deger", ylab="Frekans")
hist(veri$unemployment, main="Issizlik (Unemployment) Histogram", col="palegreen", xlab="Deger", ylab="Frekans")
...
print("--- Shapiro-Wilk Test Sonuclari ---")
...
print(paste("GSYH Aykiri Deger Sayisi:", sum(veri$gdp < alt_sinir | veri$gdp > ust_sinir)))
...
print(paste("Military Aykiri Deger Sayisi:", sum(veri$military < alt_sinir | veri$military > ust_sinir)))
...
print(paste("Homicides Aykiri Deger Sayisi:", sum(veri$intentional_homicides < alt_sinir | veri$intentional_homicides > ust_sinir)))
...
print(paste("Unemployment Aykiri Deger Sayisi:", sum(veri$unemployment < alt_sinir | veri$unemployment > ust_sinir)))
...
#Korelasyon Matrisi Gorseli
ggcorrplot(kor_matris, 
           method = "circle", 
           lab = TRUE, 
           title = "Degiskenler Arasi Korelasyon Matrisi",)
...
#WSS Grafi??i
plot(1:10, wss, type = "b", pch = 19, col = "purple", 
...
veri_normalize #hangi gozlemler hangi kumede yer aliyor?
head(veri_normalize)
